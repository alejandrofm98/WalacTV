...
interface ChannelDao {
    ...
    @Query("SELECT * FROM channels WHERE countries LIKE '%' || :country || '%' ORDER BY numero ASC LIMIT :limit OFFSET :offset")
    ...
    @Query("SELECT * FROM channels WHERE countries LIKE '%' || :country || '%' AND grupoNormalizado = :group ORDER BY numero ASC LIMIT :limit OFFSET :offset")
    ...
    @Query("SELECT * FROM channels WHERE nombreNormalizado LIKE '%' || :query || '%' AND countries LIKE '%' || :country || '%' ORDER BY numero ASC LIMIT 100")
    ...
    @Query("SELECT COUNT(*) FROM channels WHERE countries LIKE '%' || :country || '%' AND grupoNormalizado = :group")
    suspend fun getCountByCountryAndGroup(country: String, group: String): Int

    @Query("SELECT COUNT(*) FROM channels WHERE countries LIKE '%' || :country || '%'")
    ...
    @Query("SELECT DISTINCT countries FROM channels WHERE countries != '' ORDER BY countries")
    ...
    @Query("SELECT DISTINCT grupoNormalizado FROM channels WHERE countries LIKE '%' || :country || '%' AND grupoNormalizado != '' ORDER BY grupoNormalizado")
...
interface MovieDao {
    ...
    @Query("SELECT * FROM movies WHERE countries LIKE '%' || :country || '%' ORDER BY nombreNormalizado ASC LIMIT :limit OFFSET :offset")
    ...
    @Query("SELECT * FROM movies WHERE countries LIKE '%' || :country || '%' AND grupoNormalizado = :group ORDER BY nombreNormalizado ASC LIMIT :limit OFFSET :offset")
    ...
    @Query("SELECT * FROM movies WHERE nombreNormalizado LIKE '%' || :query || '%' AND countries LIKE '%' || :country || '%' ORDER BY nombreNormalizado ASC LIMIT 100")
    ...
    @Query("SELECT * FROM movies WHERE nombreNormalizado LIKE '%' || :query || '%' AND countries LIKE '%' || :country || '%' AND grupoNormalizado = :group ORDER BY nombreNormalizado ASC LIMIT 100")
    suspend fun searchByCountryAndGroup(query: String, country: String, group: String): List<MovieEntity>

    @Query("SELECT COUNT(*) FROM movies")
    suspend fun getCount(): Int

    @Query("SELECT COUNT(*) FROM movies WHERE countries LIKE '%' || :country || '%' AND grupoNormalizado = :group")
    suspend fun getCountByCountryAndGroup(country: String, group: String): Int

    @Query("SELECT COUNT(*) FROM movies WHERE countries LIKE '%' || :country || '%'")
    suspend fun getCountByCountry(country: String): Int

    @Query("SELECT COUNT(*) FROM movies WHERE grupoNormalizado = :group")
    suspend fun getCountByGroup(group: String): Int

    @Query("SELECT DISTINCT countries FROM movies WHERE countries != '' ORDER BY countries")
    suspend fun getDistinctCountries(): List<String>

    @Query("SELECT DISTINCT grupoNormalizado FROM movies WHERE grupoNormalizado != '' ORDER BY grupoNormalizado")
    suspend fun getDistinctGroups(): List<String>

    @Query("SELECT DISTINCT grupoNormalizado FROM movies WHERE countries LIKE '%' || :country || '%' AND grupoNormalizado != '' ORDER BY grupoNormalizado")
    suspend fun getDistinctGroupsByCountry(country: String): List<String>
...
@Dao
interface SeriesDao {
    @Query("SELECT MIN(id) as id, MIN(providerId) as providerId, MIN(logo) as logo, MIN(countries) as countries, 0 as temporada, 0 as episodio, serieName, serieName as nombreNormalizado, MIN(grupoNormalizado) as grupoNormalizado FROM series GROUP BY serieName ORDER BY serieName ASC LIMIT :limit OFFSET :offset")
    suspend fun getAllPaged(limit: Int, offset: Int): List<SeriesEntity>

    @Query("SELECT MIN(id) as id, MIN(providerId) as providerId, MIN(logo) as logo, MIN(countries) as countries, 0 as temporada, 0 as episodio, serieName, serieName as nombreNormalizado, MIN(grupoNormalizado) as grupoNormalizado FROM series WHERE countries LIKE '%' || :country || '%' GROUP BY serieName ORDER BY serieName ASC LIMIT :limit OFFSET :offset")
    ...
    @Query("SELECT MIN(id) as id, MIN(providerId) as providerId, MIN(logo) as logo, MIN(countries) as countries, 0 as temporada, 0 as episodio, serieName, serieName as nombreNormalizado, MIN(grupoNormalizado) as grupoNormalizado FROM series WHERE countries LIKE '%' || :country || '%' AND grupoNormalizado = :group GROUP BY serieName ORDER BY serieName ASC LIMIT :limit OFFSET :offset")
    suspend fun getByCountryAndGroupPaged(country: String, group: String, limit: Int, offset: Int): List<SeriesEntity>

    @Query("SELECT id, providerId, logo, countries, temporada, episodio, serieName, nombreNormalizado, grupoNormalizado FROM series WHERE serieName LIKE '%' || :query || '%' LIMIT 500")
    suspend fun search(query: String): List<SeriesEntity>

    @Query("SELECT id, providerId, logo, countries, temporada, episodio, serieName, nombreNormalizado, grupoNormalizado FROM series WHERE serieName LIKE '%' || :query || '%' AND countries LIKE '%' || :country || '%' LIMIT 500")
    suspend fun searchByCountry(query: String, country: String): List<SeriesEntity>

    @Query("SELECT id, providerId, logo, countries, temporada, episodio, serieName, nombreNormalizado, grupoNormalizado FROM series WHERE serieName LIKE '%' || :query || '%' AND grupoNormalizado = :group LIMIT 500")
    suspend fun searchByGroup(query: String, group: String): List<SeriesEntity>

    @Query("SELECT id, providerId, logo, countries, temporada, episodio, serieName, nombreNormalizado, grupoNormalizado FROM series WHERE serieName LIKE '%' || :query || '%' AND countries LIKE '%' || :country || '%' AND grupoNormalizado = :group LIMIT 500")
    ...
    @Query("SELECT COUNT(DISTINCT serieName) FROM series WHERE countries LIKE '%' || :country || '%' AND grupoNormalizado = :group")
    suspend fun getCountByCountryAndGroup(country: String, group: String): Int

    @Query("SELECT COUNT(DISTINCT serieName) FROM series WHERE countries LIKE '%' || :country || '%'")
    suspend fun getCountByCountry(country: String): Int

    @Query("SELECT COUNT(DISTINCT serieName) FROM series WHERE grupoNormalizado = :group")
    suspend fun getCountByGroup(group: String): Int

    @Query("SELECT DISTINCT countries FROM series WHERE countries != '' ORDER BY countries")
    suspend fun getDistinctCountries(): List<String>

    @Query("SELECT DISTINCT grupoNormalizado FROM series WHERE grupoNormalizado != '' ORDER BY grupoNormalizado")
    suspend fun getDistinctGroups(): List<String>

    @Query("SELECT DISTINCT grupoNormalizado FROM series WHERE countries LIKE '%' || :country || '%' AND grupoNormalizado != '' ORDER BY grupoNormalizado")
    suspend fun getDistinctGroupsByCountry(country: String): List<String>
