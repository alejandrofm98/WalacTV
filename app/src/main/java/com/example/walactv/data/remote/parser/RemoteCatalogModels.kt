package com.example.walactv.data.remote.parser
...
fun parseRemoteFilterOptions(payload: JSONObject, key: String): List<FilterOptionDto> {
            ...
            when (rawValue) {
                is JSONObject -> {
                    ...
                    if (value.isNotBlank() && label.isNotBlank()) {
                        add(FilterOptionDto(value = value, label = label))
                    }
                }

                else -> {
                    val value = values.optString(index).trim()
                    if (value.isNotBlank()) add(FilterOptionDto(value = value, label = value))
                }
            }
        }
    }
        .distinctBy(FilterOptionDto::value)
        .sortedBy(FilterOptionDto::label)
}
...
private fun JSONObject.toCatalogItem(expectedKind: ContentKind? = null): CatalogItem {
    ...
    return CatalogItem(
        ...
        voteAverage = optDoubleValue("vote_average")
            ?: optDoubleValue("rating"),
...
private fun JSONObject.logVodTmdbParse(
    ...
    if (title.contains("colores", ignoreCase = true)) {
        safeLogD("TMDB_PARSE", "rawColoresJson=${toString().take(4000)}")
    }
}
